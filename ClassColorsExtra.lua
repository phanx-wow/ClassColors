...
local db
local extras = CreateFrame("Frame", nil, InterfaceOptionsFramePanelContainer)
ClassColors.Extras = extras
...
function extras:Load()
	...
	hooksecurefunc("FriendsList_Update", function()
		...
		for i = 1, FRIENDS_TO_DISPLAY do
			_, _, class, _, _, _, _, _ = GetFriendInfo(offset + i)

			if class then
				local class = CUSTOM_CLASS_COLORS:GetClassToken(class)
				local color = CUSTOM_CLASS_COLORS[class]
				_G["FriendsFrameFriendButton"..i.."ButtonTextName"]:SetColor(color.r, color.g, color.b)
			end
		end
	end)
